...
$gp1_sql	 = "
		...
		SELECT 
		a.comment_kind,
		sum(a.timeSpentSeconds)/3600 as timewant,count(a.timeSpentSeconds) as cnt,
		
		(
			select AVG(b.timeSpentSeconds/3600) from jirasync_work as b where b.comment_kind = a.comment_kind 
		) as wks
...
while($gp1_info	 = mysqli_fetch_array($gp1_res)){
	...
	$gp1_text = $gp1_text."['".$gp1_info['comment_kind']."',".$worktime.",".$gp1_info['wks']."],";
	array_push($want_list,
				array($gp1_info['comment_kind'], $gp1_info['timewant'], $gp1_info['cnt']) );
...
SELECT 
comment_kind,sum(timeSpentSeconds)/60 as timewant,count(timeSpentSeconds) as cnt
...
while($gp1_info	 = mysqli_fetch_array($gp1_res)){
	...
	echo "
	<tr>
		<td data-field='s_99' data-sortable='true' >".$gp1_info['comment_kind']."</td>
